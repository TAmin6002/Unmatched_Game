...
void Ftxui_Front::Players_Info_List(Player *p1, Player *p2)
{
    ...
    auto safe_stoi = [](const std::string &s) -> int
    {
        ...
        {
            // Empty field or non-numeric input: fall back instead of crashing.
            return 0;
        }
    };

    p1->set_name(name_p1.empty() ? "Player 1" : name_p1), p1->set_age(safe_stoi(age_p1));
    p2->set_name(name_p2.empty() ? "Player 2" : name_p2), p2->set_age(safe_stoi(age_p2));
...
Element Dracula_Box(Player *p1, Player *p2)
{
    ...
    return vbox({
            ...
            //    text("Move : " + to_string(dracula_player->get_character()->get_Movement())),
               ...
               separator(),
               text("SISTERS") |bold |color(Color::RGB(210, 170, 255)),
